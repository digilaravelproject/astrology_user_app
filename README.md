...
Get-ChildItem -Recurse -Include *.dart -Path lib | ForEach-Object {
    (Get-Content $_) -replace 'package:file_stracture','package:astro_user' | Set-Content $_
}
```
...
* All imports updated from `package:file_stracture/...` → `package:astro_user/...`
